...
#+begin_src emacs-lisp
  ...
  (defun clover/exwm-update-title ()
    (pcase exwm-class-name
      ("firefox" (exwm-workspace-rename-buffer (format "FX: %s" exwm-title)))
      ("Brave-browser" (exwm-workspace-rename-buffer (format "BB: %s" exwm-title)))))

  (defun clover/exwm-lock-screen ()
    (start-process-shell-command "lock" nil "slock"))

  (defun clover/configure-window-by-class ()
    (interactive)
  ...
  (use-package exwm
    ...
    ;; When window "title" update, use it to set the buffer name
    (add-hook 'exwm-update-title-hook #'clover/exwm-update-title)

    ;; Configure windows as they're created
    (add-hook 'exwm-manage-finish-hook #'clover/configure-window-by-class)
...
>>>>>>> a41e20c (EXWM improvements):.emacs.d/configuration.org
